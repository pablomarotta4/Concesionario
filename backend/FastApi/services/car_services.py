...
def create_car(car: Car) -> Car:
    try:
        ...
        result = get_cars_collection().insert_one(car_dict)
        car_id = str(result.inserted_id)

        if car_history is not None:
            create_car_history(car_id, car_history)

        new_car_data = get_cars_collection().find_one({"_id": result.inserted_id})
...
def update_car(car_id: str, car: Car) -> Car:
    try:
        ...
        get_cars_collection().update_one({"_id": ObjectId(car_id)}, {"$set": car_dict})
        if car_history is not None:
...
def delete_car(car_id: str, current_user: User = Depends(get_current_user)) -> bool:
    try:
        ...
        result = get_cars_collection().delete_one({"_id": ObjectId(car_id)})
...
def get_car_by_id(car_id: str) -> Optional[Car]:
    try:
        car_data = get_cars_collection().find_one({"_id": ObjectId(car_id)})
        if car_data:
            car = car_schema(car_data)
            car["id"] = str(car_data["_id"])

            history_data = get_database().car_histories.find_one({"car_id": car["id"]})
            if history_data:
...
def get_all_cars() -> List[Car]:
    try:
        cars_data = get_cars_collection().find()
